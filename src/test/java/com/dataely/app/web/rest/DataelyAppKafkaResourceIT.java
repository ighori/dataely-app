...
import com.dataely.app.config.KafkaProperties;
...
class DataelyAppKafkaResourceIT {
    ...
    void setup() {
        ...
        DataelyAppKafkaResource kafkaResource = new DataelyAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/dataely-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dataely-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
